...
def brownian_1d():
    ...
    line,  = ax.plot([], [], lw=2)
    point  = ax.scatter([], [], s=64, zorder=5)
    ...
    def update(frame):
        ...
        point.set_offsets([[t[frame], W[frame]]])
...
def multiple_brownian():
    ...
    _y_lo, _y_hi = ax_traj.get_ylim()
    time_marker = LineCollection([[(t[0], _y_lo), (t[0], _y_hi)]], lw=1, alpha=0.5, color='C0')
    ax_traj.add_collection(time_marker)
    ...
    def init():
        ...
        for ln in lines:
            ln.set_data([], [])
        time_marker.set_segments([[(t[0], _y_lo), (t[0], _y_hi)]])
    ...
    def update(frame):
        ...
        time_marker.set_segments([[(t[frame], _y_lo), (t[frame], _y_hi)]])
...
def heatmap():
    ...
    _y_lo, _y_hi = ax_traj.get_ylim()
    time_marker = LineCollection([[(t[0], _y_lo), (t[0], _y_hi)]], lw=1, alpha=0.6, color='C0')
    ax_traj.add_collection(time_marker)
